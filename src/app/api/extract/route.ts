...
// Allow up to 60s for AI extraction + email generation (Pro plan)
// On Hobby plan, streaming extends timeout from 10s → 25s
export const maxDuration = 60;

export async function POST(request: NextRequest) {
  // ─── Auth & Validation (fast, before streaming) ─────────
  ...
  const {
    ...
    groqApiKey?: string;
    language?: "id" | "en";
    selectedModel?: { provider: "gemini" | "groq"; modelId: string } | null;
  };
  ...
  // ─── Streaming Response (extends timeout on Vercel) ─────
